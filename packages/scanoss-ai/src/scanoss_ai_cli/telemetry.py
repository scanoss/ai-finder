...
def _get_config_path() -> Path:
    """Get the scanoss-ai config directory path."""
    return Path.home() / ".scanoss-ai"


def _check_config_opt_out() -> bool:
    """Check if telemetry is disabled via config file at ~/.scanoss-ai/config.json."""
...
def _is_disabled() -> bool:
    ...
    if _disabled_by_flag:
        return True
    if os.environ.get("SCANOSS_AI_TELEMETRY") == "0":
        return True
